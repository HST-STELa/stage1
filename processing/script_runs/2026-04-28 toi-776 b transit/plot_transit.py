...
all_lc = spec_records
ex = phase_axis_extent(all_lc)

xs, ys, yerr, xhalf = lc_trace_arrays(all_lc)
...
if len(yerr) and np.any(np.isfinite(np.asarray(yerr, dtype=np.float64))):
    _kw_lc['error_y'] = dict(type='data', array=np.asarray(yerr, dtype=np.float64))

fig_lc = go.Figure()
fig_lc.add_trace(go.Scatter(x=xs, y=ys, **_kw_lc))

if ex:
    fig_lc.update_xaxes(range=[ex[0], ex[1]])
fig_lc.update_xaxes(title_text='Phase (h)')
fig_lc.update_yaxes(title_text='Integrated flux', tickformat='.2~e')
