...
from processing import processing_utilities as pcutils
...
with warnings.catch_warnings():
    ...
    fh = staging_folder / '..' / 'host_catalog.ecsv'
    hosts = table.QTable.read(fh)
...
#%% load in and standardize sarah's specs
    ...
    dem_files = sorted(folder_dem.rglob(f'*{targname_girish}*.fits'))
...
cmp_fd = staging_folder / 'comparison_plots'
if not cmp_fd.exists():
    os.mkdir(cmp_fd, exists_ok=True)
...
for target in targets:
    ...
    sh.copy(compfile, cmp_fd / compfile.name)