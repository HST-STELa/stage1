...
min_samples = 4*4*4*3*4*5 # used as a check later to ensure all grid pts of Ethan's sims were sampled
...
def calculate_snr_tables_for_target(target, overwrite=False, selected_tst_types=tst_types):
    ...
    for planet in utils.printprogress(host.planets, 'dbname', prefix='\tplanet '):
        for tst_type in selected_tst_types:
            ...
            if tst_type == 'model':
                chooser = tutils.best_by_det_frac
                chooser_kws = dict(detection_threshold=sigma_threshold)
            elif tst_type == 'flat':
                chooser = tutils.best_by_mean_snr
                chooser_kws = dict()
            else:
                raise ValueError
            ...
            def build_planet_snrs(grating, base_aperture, all_apertures):
                snrs = build_snrs(
                    ...
                    offsets,
                    max_safe_offset,
                    fn_to_choose_best=chooser,
                    chooser_fn_kws=chooser_kws,
                    verbose=True,
                )
                return snrs
