...
#%% look at trace locations, update with median of nodding positions

    rows = [(k, v) for k, v in tracelocs.items()]
    from astropy import table
    tracetbl = table.Table(rows=rows, names='id y'.split())
    tracetbl.sort('id')
    tracetbl.pprint(-1,-1)

    n = len(tracetbl)
    idxA = sorted(list(range(0,n,4)) + list(range(3,n,4)))
    idxB = sorted(list(range(1,n,4)) + list(range(2,n,4)))
    overlap = set(idxA) & set(idxB)
    assert len(overlap) == 0
    leftover =  set(list(range(n))) - set(idxA) - set(idxB)
    assert len(leftover) == 0
    tracelocA = np.median(tracetbl['y'][idxA])
    tracelocB = np.median(tracetbl['y'][idxB])

    tracetbl['y'][idxA] = tracelocA
    tracetbl['y'][idxB] = tracelocB
