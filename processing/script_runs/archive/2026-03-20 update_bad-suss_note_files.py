...
helpful_message = (
    "This file is just meant to alert users at a glance that certain "
    "files might be unusable or suspect. We keep files with bad data because time "
    "has shown that we often want to revisit bad or suspect data to "
...
files = (
    sorted(paths.data_targets.rglob('_.all clear.*.txt')) +
    sorted(paths.data_targets.rglob('_.has issues.*.txt'))
)
for f in files:
...
#%% actually just delete them all, things are too messed up

files = sorted(paths.data_targets.rglob('_.*.txt'))
for f in files:
    os.remove(f)
