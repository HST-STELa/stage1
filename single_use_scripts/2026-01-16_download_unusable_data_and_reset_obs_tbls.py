...
#%% create or load table of observation information
    ...
    try:
        ...
        targets_w_no_obs_tbl.append(target)
        obs_tbl = obs_tbl_tools.initialize(files_science)
        print(f'\nObservation table initialized for {target}:\n')
    viewcols = 'science config,start,archive id,usable,reason unusable,notes,flags'.split(',')
...
#%% clear usability columns, piggyback changing key cos files to x1d instead of rawtag

    unusable_mask = ~obs_tbl['usable'].filled(True)
    obs_tbl['usable'].mask[unusable_mask] = True
    obs_tbl['reason unusable'].mask[unusable_mask] = True

    obs_tbl['usability status'] = table.MaskedColumn(['unchecked']*len(obs_tbl), dtype='O')
    usable_mask = obs_tbl['usable'].filled(False)
    obs_tbl['usability status'][usable_mask] = 'all clear'

    for i, row in enumerate(obs_tbl):
        if 'cos' in row['science config']:
            filenames = list(row['key science files'])
            root = filenames[0].split('_')[0]
            newnames = [f for f in filenames if 'rawtag' not in f]
            x1d = f'{root}_x1d.fits'
            if x1d not in newnames:
                newnames.append(x1d)
            obs_tbl['key science files'][i] = newnames
    viewcols.append('usability status')
...
#%% find all data
    ...
    print(f'\nFinding missing science files in the archive for {target}.')
...
targets_w_new_data = list(set(targets_w_new_data))
...
#%% print any targets that don't have an obs tbl

print('These targets had no obs tbl:')
print(targets_w_no_obs_tbl)
