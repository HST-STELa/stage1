...
#%% prep for target processing
    ...
    # Human acq visibility notes before clear (old + current menu wordings).
    preserved_acq_identify_notes = {}
    _can_see_substrings = (
        'was able to identify target in acquisition image',
        'identified target in acquisition image',
    )
    _cannot_see_substrings = (
        'could not identify target in acquisition image',
        'deemed target absent in acquisition image',
    )
    for row in obs_tbl:
        aid = row['archive id']
        for note in row.get('notes', []):
            if any(s in note for s in _cannot_see_substrings):
                preserved_acq_identify_notes[aid] = note
                break
            if any(s in note for s in _can_see_substrings):
                preserved_acq_identify_notes[aid] = note
                break
...
#%% verify that acq files are present for every observation
    ...
    known_missing = obs_tbl.substring_match_mask('notes', 'no acquisition found')
    for row in obs_tbl:
    ...
    for acq_name in acq_filenames:
        ...
        if image_shown:
            ...
            if human_review_acq == 'yes' or (human_review_acq == 'issues' and acq_issues):
                preexisting_acq_identify_note = None
                for aid in obs_tbl['archive id'][assoc_obs_mask]:
                    if aid in preserved_acq_identify_notes:
                        preexisting_acq_identify_note = preserved_acq_identify_notes[aid]
                        break
                if preexisting_acq_identify_note is not None:
                    print('Using saved acquisition visibility note (skipping interactive review).')
                    if any(s in preexisting_acq_identify_note for s in _cannot_see_substrings):
                        acq_issues = True
                        msgs.append(
                            obt.notes_menu['cannot see target in acq'].format(user=human_reviewer)
                        )
                    else:
                        msgs.append(
                            obt.notes_menu['can see target in acq'].format(user=human_reviewer)
                        )
                else:
