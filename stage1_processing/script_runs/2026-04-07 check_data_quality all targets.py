...
#%% clear targets if I found flux was present but automated checker raised concern
...
for target in targets:
    ...
    for row in obstbl:
        ...
        if len(flags) == 1 and flags[0] == 'acquisition untrustworthy':
            any_other_acq_notes = False
            low_flux = False
            human_pass = True
            for note in notes:
                if note.startswith('flux within'):
                    sigma, = re.findall(r'(\d+.\d+) sigma', note)
                    sigma = float(sigma)
                    if sigma < obstbl.meta['acq signal check sigma threshold']:
                        low_flux = True
                elif 'could not identify target' in note:
                    human_pass = False
                elif 'acquisition' in note:
                    any_other_acq_notes = True
            if low_flux and human_pass:
                obstbl.add_note(row.index, obt.notes_menu['can see target in acq'].format(user=human_reviewer))
                modified = True
                if not any_other_acq_notes:
                    fix_mask.append(True)
                    continue
        fix_mask.append(False)
