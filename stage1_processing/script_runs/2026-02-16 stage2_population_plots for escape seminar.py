...
eval_filenames = ['stage2_evaluation_metrics_2026-02-??_vwind=150_snr-thresh=1.ecsv',
                  'stage2_evaluation_metrics_2026-02-??_vwind=400_snr-thresh=1.ecsv']
wind_labels = 'vwind=150 vwind=400'.split()

path_seminar_presentation = Path('/Users/parke/Google Drive/Presentations/2026-02-18 Escape Seminar')
...
for windlbl, eval_filename in zip(wind_labels, eval_filenames):
    
    #%% load table and make labels
    
    eval_path = paths.catalogs / 'archive' / eval_filename
    ...
    def make_detectability_plot(ykey, yfloor=0):
        ...
        def plot_set(mask, *args, addlbls=False, dim_rocks=True, **kws):
            ...
            if dim_rocks:
                ...
                ax.annotate('translucent = rocky', xy=(0.98,0.33), xycoords='axes fraction',
                            ha='right', va='center',
        ...
        lnd = plot_set(masks['detections'], addlbls=False, dim_rocks=True, **pt_styles['detections'])
        lnt = plot_set(masks['tentative'], addlbls=False, dim_rocks=True, **pt_styles['tentative'])
        lnn = plot_set(masks['nondetections'], addlbls=False, dim_rocks=True, **pt_styles['nondetections'])
        lnu = plot_set(masks['unknown'], addlbls=False, dim_rocks=True, **pt_styles['unknown'])
    ...
    frac_fig.savefig(path_seminar_presentation / f'{windlbl} detectability fraction vs lya flux plot.png')
    ...
    flat_fig.savefig(path_seminar_presentation / f'{windlbl} flat transit SNR vs lya flux plot.png')
    ...
    def make_scatter(skey, scalefunc=None):
        ...
        plot_set(masks['detections'], addlbls=False, **pt_styles['detections'])
        plot_set(masks['tentative'], addlbls=False, **pt_styles['tentative'])
        plot_set(masks['nondetections'], addlbls=False, **pt_styles['nondetections'])
        plot_set(masks['unknown'], addlbls=False, **pt_styles['unknown'])
    ...
    frac_fig_scat.savefig(path_seminar_presentation / f'{windlbl} detectability fraction population.png')
    ...
    flat_fig_scat.savefig(path_seminar_presentation / f'{windlbl} flat transit SNR population.png')