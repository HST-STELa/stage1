...
#%% prep for target processing
    ...
    # Human acq visibility notes (before any clear) so we can skip re-prompting after clear_usability.
    preserved_acq_identify_notes = {}
    _can_see_sub = 'was able to identify target in acquisition image'
    _cannot_see_sub = 'could not identify target in acquisition image'
    for i in range(len(obs_tbl)):
        aid = obs_tbl['archive id'][i]
        for note in obt.ObsTable._iter_nonnull_cell_items(obs_tbl['notes'][i]):
            text = note if isinstance(note, str) else str(note)
            if _can_see_sub in text:
                preserved_acq_identify_notes[aid] = text
                break
            if _cannot_see_sub in text:
                preserved_acq_identify_notes[aid] = text
                break
        ...
        if image_shown:
            ...
            if human_review_acq == 'yes' or (human_review_acq == 'issues' and acq_issues):
                ...
                if preexisting_acq_identify_note is not None:
                    ...
                    if _cannot_see_sub in preexisting_acq_identify_note:
                        acq_issues = True
                        msgs.append(preexisting_acq_identify_note)
                    else:
...
#%% redo these using human review for acq issues
