...
toggle_save_new_stela_names = False # necessary to avoid errors if, e.g., new hosts have shown up in exoplanet archvive
...
toggle_remake_filtered_hst_archive = False  # needed if it has been a while since the last remake
toggle_refresh_archival_transit_list = False
archival_transit_hand_remove = []
archival_transit_hand_add = [
    'K2-18', # currently K2-18 phase in obs table has zero error so it fails auto transit finder
    'HD 219134', # currently fails because insufficient pre-transit exposure
]
...
"""
def match_agreement(x, y):
    agreement = (
        (len(x) == 0 and len(y) == 1) or
        (len(x) == 1 and len(x) == 0) or
        (x == y)
    )
    return agreement

xmatch_compare = list(map(match_agreement, i_toi_matches_tic_id, i_toi_matches_position))
if not all(xmatch_compare):
    raise NotImplementedError('The TIC ID and position cross matches do not agree. Revisions required.')
# as of 2026-03-10 there is one planet with a position match but no tic id match and I let this pass
# this is Kepler-635 b, which matches a TOI (but not sure which), not likely to be selected anyway
# and I think I trust the TIC ID match more
...
# missing TIC ID for TOI-2267
cat.loc['TOI-2267 B']['tic_id'] = 459837008
...
print(f'{np.sum(transfer)} of {n_bad_st_rad} bad stellar radius values filled based on Teff ({count_hosts(transfer)} unique hosts).')
if toggle_plots:
...
print(f'{np.sum(transfer)} of {n_bad_st_mass} bad stellar mass values filled based on Teff ({count_hosts(transfer)} unique hosts).')
if toggle_plots:
...
transfer = (M > 0) & cat['st_mass'].mask
print(f'{np.sum(transfer)} of {n_bad_st_mass} bad stellar mass values filled based on stellar radius ({count_hosts(transfer)} unique hosts).')
if toggle_plots:
...
print(f'{np.sum(transfer)} of {n_bad_st_teff} bad stellar teff values filled based on radius ({count_hosts(transfer)} unique hosts).')
if toggle_plots:
...
print(f'{np.sum(transfer)} of {n_bad_st_lum} bad stellar luminosity values filled based on effective temperature ({count_hosts(transfer)} unique hosts).')
...
count_tois = lambda mask: np.sum(cat['pl_name'].mask & mask)
...
a = a.to('AU')
print(f'{np.sum(transfer)} of {n_bad_orbsmax} bad planet orbsmax values filled based on orbital period ({count_tois(transfer)} TOIs).')
...
a = a.to('AU')
assert np.all(a[transfer] > 0)
print(f'{np.sum(transfer)} of {n_bad_orbsmax} bad planet orbsmax values filled based on equilibrium temperature ({count_tois(transfer)} TOIs).')
if toggle_plots:
...
print(f'{np.sum(transfer)} of {n_bad_orbsmax} bad planet Teq values filled based on smax and luminosity ({count_tois(transfer)} TOIs).')
if toggle_plots:
...
Teq = Teq.to('K')
assert np.all(Teq[transfer] > 0)
print(f'{np.sum(transfer)} of {n_bad_orbsmax} bad planet Teq values filled based on smax, stellar radius, and Teff ({count_tois(transfer)} TOIs).')
if toggle_plots:
...
print(f'{np.sum(transfer)} of {n_bad_mass} bad planet mass values filled based on radius in Chen and Kipping 2017 relationships.'
      f'\nBEWARE for gas giants a mass of 150 Mearth was assumed. After stage 1, only measured masses should be used. ({count_tois(transfer)} TOIs)')
if toggle_plots:
...
J = Mj + 5*np.log10(d) - 5
print(f'{np.sum(transfer)} stellar J mag values filled based on effective temperature ({count_hosts(transfer)} unique hosts).')
if toggle_plots:
