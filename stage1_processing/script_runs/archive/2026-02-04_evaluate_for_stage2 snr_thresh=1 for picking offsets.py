...
n_sets = 4
i_set = 0 # so I can do poor man's multiprocessing and divide into chunks

targets_all = set(target_lists.eval_no(1)) | set(target_lists.eval_no(2)) - {'v1298tau'}

targets_all = sorted(list(targets_all))
target_sets = np.array_split(list(targets_all), n_sets)
...
#%% check that all sigmas files were updated

files = snr_files = list(paths.data_targets.rglob('*tail-model*sigmas*.ecsv'))
from datetime import datetime
threshold_date = datetime(2026, 2, 4, 16)
...
targets = sorted(list(targets_all))
...
for target in tqdm(targets):
    ...
    for planet in host.planets:
        ...
        row['stellar\nwind\nspeed'] = transit.meta.get('v_stellar_wind', 150)/1e5
