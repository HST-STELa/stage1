...
while True:
  ...
  try:
    ...
    bestfluxes.meta['radius'] = hostprops['st_rad'].to_value('Rsun').filled(nan)
    bestfluxes.meta['Teff'] = hostprops['st_teff'].to_value('K').filled(nan)
    bestfluxes.meta['SpT'] = get_prop('st_spectype', '?')
    bestfluxes.meta['logg'] = get_prop('st_logg', nan)
    bestfluxes.meta['mass'] = hostprops['st_mass'].to_value('Msun').filled(nan)
